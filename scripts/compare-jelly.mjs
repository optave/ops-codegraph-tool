...
function buildNameMap(fixtureDir, lang) {
  ...
  for (const filename of fs.readdirSync(fixtureDir)) {
    ...
    let classDepth = 0; // brace depth at which the class was opened
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (classMatch) {
        ...
        classDepth = braceDepth; // depth *before* the opening brace on this line
        nameMap.set(key, classMatch[1]);
        // fall through to update braceDepth for the opening { on this line
      }

      // Update brace depth for the entire line (after class detection)
      for (const ch of line) {
        if (ch === '{') braceDepth++;
        else if (ch === '}') {
          braceDepth--;
          // If we drop back to the depth at which the class opened, the class block closed
...
if (jsonFlag) {
  ...
  console.log('\n\n## Jelly Precision/Recall (vs expected-edges.json ground truth)\n');
