...
describe('directed modularity delta — exact regression', () => {
  it('matches the pinned quality/assignment for the two-triangle directed graph', () => {
    ...
    const clusters = detectClusters(g, { directed: true, randomSeed: 2 });
    expect(clusters.quality()).toBe(0.42603550295857995);
    expect([...A, ...B].map((id) => clusters.getClass(id))).toEqual([1, 1, 1, 0, 0, 0]);
  });

  it('matches the pinned quality/assignment for the directed graph with self-loops', () => {
    const g = new CodeGraph();
    const A = ['0', '1', '2'];
    const B = ['3', '4', '5'];
    for (const id of [...A, ...B]) g.addNode(id);
    ...
    const clusters = detectClusters(g, { directed: true, randomSeed: 2 });
    expect(clusters.quality()).toBe(0.44875346260387805);
    expect([...A, ...B].map((id) => clusters.getClass(id))).toEqual([1, 1, 1, 0, 0, 0]);
  });
});
