...
**Stale lock files** (`.codegraph/*.lock` older than 1 hour): Before removing, check whether any process still holds the file. On Linux/macOS use `lsof "$f"` to verify. If the check is unavailable or the file is held, **require user confirmation** instead of deleting automatically — concurrent Claude Code sessions may hold legitimate long-lived locks.
