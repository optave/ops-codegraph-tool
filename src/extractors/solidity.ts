  ...
  if (body) {
    for (let i = 0; i < body.childCount; i++) {
      ...
      if (child.type === 'function_definition') {
        ...
        if (evName) {
          members.push({ name: evName.text, kind: 'property', line: child.startPosition.row + 1 });
        }
      } else if (child.type === 'error_declaration') {
        const errName = child.childForFieldName('name');
        if (errName) {
          members.push({ name: errName.text, kind: 'property', line: child.startPosition.row + 1 });
        }
      } else if (child.type === 'modifier_definition') {
        const modName = child.childForFieldName('name');
        if (modName) {
          members.push({ name: modName.text, kind: 'method', line: child.startPosition.row + 1 });
        }
      }
    }
  }
